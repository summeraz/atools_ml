...
def assign_descriptors_monolayers(project, multi=False, override=False,
                                  ch3=False, chain=False):
    if ch3 and chain:
        raise Exception('Only one of `ch3` and `chain` may be True.')
    root_dir = project.root_directory()
    for job in project.find_jobs():
        if override or not os.path.isfile(os.path.join(job.ws, 'descriptors-h.json')):
            if multi:
                descriptors = []
                for i, terminal_group in enumerate(job.sp['terminal_groups']):
                    if ch3:
                        smiles = job.document['ch3-smiles-{}'.format(i+1)]
                    elif chain:
                        smiles = job.document['chain-smiles-{}'.format(i+1)]
                    else:
                        smiles = job.document['h-smiles-{}'.format(i+1)]
                    ch3_smiles = job.document['ch3-smiles-{}'.format(i+1)]
                    descriptors.append(rdkit_descriptors(
                        smiles=smiles,
                        include_h_bond=True,
                        ch3_smiles=ch3_smiles
                        ))
                ...
                '''
                if(job.sp['terminal_groups'][0] == 'amino' and
                        job.sp['terminal_groups'][1] == 'carboxyl'):
                    print('{}-{}: {}, {}, {}, {},{}'.format(
                        *job.sp['terminal_groups'],
                        hbonds,
                        df['hdonors'][0],
                        df['hacceptors'][0],
                        df['hdonors'][1],
                ...
                maxs = maxs.rename({label: '{}-max'.format(label)
                                    for label in maxs.index})
                diff = diff.rename({label: '{}-diff'.format(label)
                                    for label in diff.index})
                full_descriptors = pd.concat([means, mins, maxs, diff])
                full_descriptors['hbonds'] = hbonds
                if ch3:
                    full_descriptors.to_json(os.path.join(job.ws, 'descriptors-ch3.json'))
                elif chain:
                    full_descriptors.to_json(os.path.join(job.ws, 'descriptors-chain.json'))
                else:
                    full_descriptors.to_json(os.path.join(job.ws, 'descriptors-h.json'))
            else:
                if ch3:
                    smiles = job.document['ch3-smiles']
                elif chain:
                    smiles = job.document['chain-smiles']
                else:
                    smiles = job.document['h-smiles']
                ch3_smiles = job.document['ch3-smiles']
                descriptors = rdkit_descriptors(
                        smiles=smiles,
                        include_h_bond=True,
                        ch3_smiles=ch3_smiles
                        )
                df = pd.Series(descriptors)
                hbond_tb = max(df['hdonors'], df['hacceptors']) \
                           if all((df['hdonors'], df['hacceptors'])) \
                           else 0
                hbonds = hbond_tb * 2
                df.drop(['hdonors', 'hacceptors'], 'rows', inplace=True)
                df['hbonds'] = hbonds
                if ch3:
                    df.to_json(os.path.join(job.ws, 'descriptors-ch3.json'))
                elif chain:
                    df.to_json(os.path.join(job.ws, 'descriptors-chain.json'))
                else:
                    df.to_json(os.path.join(job.ws, 'descriptors-h.json'))
